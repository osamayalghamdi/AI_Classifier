...
**RESOLVED 2026-08-02: manager approved the member-level rule — implemented in
`e5719e0` (see addendum below).**
...
**RESOLVED 2026-08-02: manager approved member texts in the payload — implemented
in `e5719e0` (see addendum below).**
...
## ADDENDUM (same pass, manager decision 2026-08-02 — implemented in `e5719e0`)

Both gaps above were closed by manager decision. Verification (real output):

**Member-level purity rule** (suboffering_cluster.py `compute_member_flags`):
member FM ∉ cluster top-2 FM codes → member `needs_review` in
`purity_flags.members`. Deterministic (count DESC, code ASC); when the 2nd/3rd
counts tie, both tied codes are minority (conservative — catches a lone wrong
member whose FM ties 2nd place). Proposal excluded only when flagged ≥ 1/3 of
members OR the cluster floor trips (2/20 < 1/3 → proposal stays, members flagged).

**Member texts in payload** (store.py `_enrich_proposal_members`):
list_proposals/get_proposal now return `members: [{id, title, description,
failure_mode}]` per member.

**Re-verification** — cache-backed re-run of proposal construction, proposal
`39d968d9013b` (pilgrim groups, 20 members):
```
cluster needs_review (excluded): False | n_fm_codes: 3 | mean_sim: 0.5794
member-level flags: 🔴 cad886 [FM-012] needs_review=True
                    🔴 390a79 [FM-009] needs_review=True
                    18× FM-018 members needs_review=False
member texts in payload: True
```
**Tests:** +5 (cad886-class flag, tie conservatism, 3-code minority, 2-code
all-top2, member texts in payload) — engine tests 21/21; full suite 91 passed
+ 15 canary-skipped.

**Residual (open items):** approve-with-exclusion and verifier-cache negative
override remain open (see rejection procedure above); the verifier's
over-generalization of "issue permit/approval" across surfaces (transport vs
Rawdah vs external-pilgrim) is a prompt-v3 limitation handled by the member
rule + human gate, NOT a prompt patch (per the canary finding).
